...
public class Parser {
        ...
        if (!(check(RIGHT_PAREN))){
            do {
                if (parameters.size()>=255){
                    throw error(peek(),"Parameter limit exceeded more than 255.");
                }
    ...
    private Expr factor() {
        Expr expr=unary();
        while(match(SLASH,STAR,MODULO,EXPONENT)){
            Token operator=previous();
            Expr right=unary();
            expr=new Expr.Binary(expr,operator,right);
        }
    ...
    private Expr finishCall(Expr callee) {
        ...
        if (!check(RIGHT_PAREN)){
            do {
                if (arguments.size()>=255){
                    throw error(peek(),"Function arguments limit exceeded more than 255 arguments");
        ...
        Token paren=consume(RIGHT_PAREN,"Expect ')' after function");
